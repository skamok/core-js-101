...
function isInsideCircle(circle, point) {
  const a = Math.abs(point.x - circle.center.x);
  const b = Math.abs(point.y - circle.center.y);
...
function reverseInteger(num) {
  ...
  return Number.parseInt(str.split('').reverse().join(''), 10);
}
...
function isCreditCardNumber(ccn) {
  ...
  const sum = arr.reduce((acc, curr, ind) => {
    const nmb = Number.parseInt(curr, 10);
    let tmp = 0;
    if ((ind + 1) % 2 === 0) {
      tmp = nmb * 2;
      if (tmp > 9) {
...
function toNaryString(num, n) {
  ...
  while (first >= n) {
    res.push(first % n);
    first = Math.floor(first / n);
    if (first < n) res.push(first);
  }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m3rows; i += 1) {
    const tmp = [];
    for (let k = 0; k < m3cols; k += 1) {
      let sum = 0;
      for (let z = 0; z < m1[i].length; z += 1) {
        sum += m1[i][z] * (m2[z][k] === undefined ? 0 : m2[z][k]);
      }
      tmp.push(sum);
    }
    m3.push(tmp);
